          ...
          <div className="grid grid-cols-12 gap-6">
            ...
            <Card className="col-span-12 flex h-56 flex-col justify-between md:col-span-4">
              ...
              <p className="font-sans text-xs text-muted-on-card">
                {kpis?.paidInvoicesCount ?? 0} facture(s) payée(s) —{' '}
                {kpis?.pendingDocumentsCount ?? 0} doc(s) en attente
              </p>
            </Card>
          </div>
                ...
                <ul className="space-y-4">
                  {featured.map((horse) => {
                    const load = horse.maxWeeklyLoadHours
                      ? Math.min(
                          100,
                          Math.round((horse.weeklyLoadHours / horse.maxWeeklyLoadHours) * 100)
                        )
                      : 0;
                    return (
                      <li key={horse.id}>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {(kpis?.pendingDocumentsCount ?? 0) > 0 ? (
              <section className="rounded-xl bg-danger/10 p-5">
                ...
                <Link
                  to="/admin/clients"
                  className="mt-3 inline-block font-sans text-xs font-semibold uppercase text-danger"
                >
                  Traiter
                </Link>
              </section>
            ...
            {criticalIncidents > 0 ? (
              <section className="rounded-xl bg-paper p-5">
                ...
                <Link
                  to="/admin/incidents"
                  className="mt-3 inline-block font-sans text-xs font-semibold uppercase text-primary"
                >
                  Ouvrir
                </Link>
              </section>
