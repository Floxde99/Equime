...
export function ClientInvoicesPage() {
  ...
  const { data: invoices = [], isPending, isError, error, refetch } = useQuery({
    queryKey: ['client-invoices'],
    queryFn: fetchClientInvoices,
  });
  const visibleInvoices = invoices.filter(
    (invoice) => invoice.status === 'sent' || invoice.status === 'paid' || invoice.status === 'overdue'
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[20rem_minmax(0,1fr)]">
        <div className="space-y-4">
          <Card>
            <p className="font-sans text-xs uppercase tracking-wide text-muted-on-card">Abonnement actuel</p>
          ...
          <div className="overflow-hidden rounded-xl">
            <img
              src={STITCH_PHOTOS.billingStables}
              alt=""
