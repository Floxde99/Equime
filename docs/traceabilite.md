...
| US-4.5 | Excuse client (Excel 3.7) → annulation avec rattrapage (US-10.3) | courses | `courseService.js` (`cancelEnrollment`), `UpcomingEnrollments.jsx` | `core.test.js` (annulation famille) | Livré |
...
| US-6.4 | Factures abo batch (Excel 12.1) | billing | Remplacé en v1.1 par la facture de saison (US-10.5, ADR 011) | — | Remplacé |
| US-6.5 | Souscription formule (Excel 8.2) | billing | Remplacé en v1.1 par le forfait par cavalier (US-10.5, ADR 011) | `subscription.test.js` | Remplacé |
...
| US-10.3 | Annulations et rattrapages (ADR 011) | courses / billing | `entitlementService.js`, `courseService.js` (`enrollRider`, `cancelEnrollment`, annulation club), `EnrollSection.jsx`, `UpcomingEnrollments.jsx` | `entitlements.test.js` (T-10.3.1 à T-10.3.8), `core.test.js`, E2E `client-flow.spec.js` | Livré |
| US-10.5 | Forfaits par cavalier et échéanciers (ADR 011) | billing | `subscriptionService.js`, `lib/seasons.js`, `billingService.js` (`recordPayment`), `paymentService.js`, `SubscriptionForm.jsx`, `RecordPaymentDialog.jsx`, `InvoiceDetailDialog.jsx`, migration `20260925100000_rider_subscriptions_payments` | `seasons.test.js`, `subscription.test.js`, `payment.test.js` (T-10.5.1 à T-10.5.8), E2E `billing-flow.spec.js` | Livré |
| US-10.8 | Ergonomie au niveau du marché (recherche client, euros, paramètres club) | admin / billing / settings | `adminService.searchFamilies`, `settingsService.js`, `combobox.jsx`, `money-input.jsx`, `CreateInvoiceForm.jsx`, `PlanManager.jsx`, `AdminSettingsPage.jsx`, `@equime/shared` (formatters, documentRules, zodLocale) | `settingsSearch.test.js`, `formatters.test.js`, `documentRules.test.js`, `zodLocale.test.js` (T-10.8.1 à T-10.8.6) | En cours |
